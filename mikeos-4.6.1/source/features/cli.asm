...
get_cmd:				; Main processing loop
	...
	mov di, nama_string		; 'NAMA' entered?
	call os_string_compare
	jc near print_nama

	mov di, nim_string		; 'NIM' entered?
	call os_string_compare
	jc near print_nim

	mov di, ttl_string		; 'TTL' entered?
	call os_string_compare
	jc near print_ttl

	mov di, alamat_string		; 'ALAMAT' entered?
	call os_string_compare
	jc near print_alamat

	mov di, nohp_string		; 'NOHP' entered?
	call os_string_compare
	jc near print_nohp

	mov di, cita_string		; 'CITA' entered?
	call os_string_compare
	jc near print_cita

	mov di, jeniskelamin_string		; 'JENISKLM' entered?
	call os_string_compare
	jc near print_jeniskelamin

	mov di, hoby_string		; 'HOBY entered?
	call os_string_compare
	jc near print_hoby
...
; =====================================================================
	...
	help_text		db 'Commands: DIR, LS, COPY, REN, DEL, CAT, SIZE, CLS, HELP, DATE, TIME, VER, EXIT, NAMA, NIM, TTL, ALAMAT, NOHP, CITA, DATE, TIME, JENISKLM, HOBY', 13, 10, 0
	...
	nama_msg		db 'Hafid Bastian',13,10,0
	nim_msg			db 'A11.2019.11762', 13,10,0
	ttl_msg			db	'Grobogan 31 Desember 2000',13,10,0
	alamat_msg		db 'Getasrejo RT 07 RW 02, Kec Grobogan, Kab Grobogan, Jateng, Indonesia',13,10,0
	nohp_msg		db '0895358021874', 13, 10, 0
	cita_msg		db 	'Membuat Studio desain dan back end di daerah saya dengan nama Slam dengan tema roket warna kombinasi kuning dan orange' ,13,10,0
	jeniskelamin_msg	db 'Laki - laki',13,10,0
	hoby_msg			db 'VolleyBall',13,10,0
	...
	list_string		db 'LS', 0
	nama_string		db 'NAMA',0
	nim_string		db 'NIM',0
	ttl_string		db 'TTL',0
	alamat_string	db 'ALAMAT',0
	nohp_string		db 'NOHP',0
	cita_string		db 'CITA',0
	jeniskelamin_string	db 'JENISKLM',0
	hoby_string			db 'HOBY', 0
